...
public class AppManager {
     ...
     * 如果处于回收状态，则可以进行重启等操作，否则才进行初始化等操作，以此规避回收后恢复界面时某些对象为空引起crash问题；
     */
    ...
    public static void jumpForResult(Class<? extends Activity> aty, Map<String, Serializable> map, int requestCode) {
        Activity context = ActivityStack.getCurrentActivity();
        Intent intent = new Intent(context, aty);
        if (map != null) {
            for (String key : map.keySet()) {
